...
- [ ] Make alderaan's setup shareable with other machines' [0/3]
  - [ ] extract xserver's setup inside each machine for one (chris does not use xmonad nor does she know qwerty)
  - [ ] Install a bunch of default packages but let the machines define their specific one (chris' machine is not a dev one)
