#+title: learning/hacking...
#+author: ardumont
#+options: hidestar

* IN-PROGRESS synology debian [8/10]
- [X] Install debian chroot
- [X] Make it persistent over syno's reboot
- [X] minidlna
- [X] znc
- [X] etckeeper
- [X] git server
...
** Hint for automated release
from https://github.com/blog/1645-releases-api-preview
#+BEGIN_SRC sh
...
- [X] 0.7.4
- [X] 0.7.5
** PENDING future release 0.7.6
* PENDING org2jekyll - https://github.com/ardumont/org2jekyll
- [X] 0.0.1
- [X] 0.0.2
- [X] 0.0.3
- [X] 0.0.4
...
- [X] 0.1.7
- [X] 0.1.8
- [ ] make a nix derivation
* TODO haskell [1/2]
** IN-PROGRESS snake game
Make a snake with my son Theo in haskell
** DONE env dev [4/4]
CLOSED: [2015-09-04 Fri 15:27]
- [X] nix - http://nixos.org/nixpkgs/manual/#how-to-create-a-development-environment
- [X] stack
- [X] cabal and nix
- [X] haskell-mode + cabal and nix
* PENDING emacs [1/2]
** TODO emacs configuration [0%]
- [ ] Create your own self contained emacs distribution
- [ ] Drop git submodules use
- [ ] Take a deeper look at nix' derivation
** DONE take a look at nix-emacs
CLOSED: [2015-09-05 Sat 07:48]
https://github.com/travisbhartwell/nix-emacs

* TODO mbsync/offlineimap [0/2]
- [ ] offlineimap.el - from julien danjou, might learn tons of stuff about process
- [ ] check out mbsync
* TODO gpg tools
- [X] signing-party (caff for signing keys)
- [ ] parcimonie (to silently update keys through tor)
* DONE nas [5/5]
CLOSED: [2015-07-22 Wed 09:34]
- [X] Add other source of packages - https://synocommunity.com/
- [X] Install znc
- [X] Make nas open to the world
- [X] Increase znc's buffer limit
- [X] Install home git server on nas
* DONE Chris's machine - alderaan [3/3]
CLOSED: [2015-07-23 Thu 09:20]
- [X] Reboot old machine
- [X] Install pci wifi card + new hdd
- [X] Install NixOS for alderaan

* TODO pass and xmonad pass
...
- [ ] Or: patch directly XMonad.Prompt.Pass with the help of http://blog.tarn-vedra.de/posts/xmonad-password-store.html
- [ ] Joachim seems to have already fixed this, so try the latest version! yeah!
- [ ] backup me own store
* DONE irc-pack reloaded
CLOSED: [2015-07-21 Tue 13:25]
- [X] Read server/port and fullname from configuration file directly
- [X] Does it work with irc bouncer setup, e.g. znc? Yes, cool.
* PENDING chat-pack reloaded!
- [X] Make it work again (only a configuration problem)
- [X] Make it a minor mode
- [X] Find and install chat-pack with otr!
- [ ] Improve packaging of mainstream jabber-otr to avoid duplication in my repo ~> nix to the rescue
* CANCELLED mail-pack/mu4e configuration -> changing to notmuch
CLOSED: [2015-07-21 Tue 13:23]
- [X] Use mu4e's refiling mechanism to try and drop gmail's filters
- [X] Add some bindings (M-x mu4e-update-index, etc...)
- [X] Agnostify refiling mechanism (there can be lots of rules...)
- [X] Add tests
- [X] Make a minor mode
- [X] Make it load (packaging hell)
- [X] Refiling now works!
- [X] Switch between accounts now works too!
- [X] Add contact auto-completion that works! (already existing, just needed to find the right setup)
- [ ] Cask: how to make a git clone a source in Cask recipe? - http://cask.readthedocs.org/en/latest/guide/dsl.html?highlight=depends#el.function.depends-on
- [ ] Due to mu4e's absence in melpa/marmalade, failure to make tests run in CI through Cask (https://github.com/djcb/mu/issues/620, https://github.com/djcb/mu/issues/370, ...)
- [ ] Tests are ok in live emacs but not in cask, make them ok
- [ ] Make them run on travis-ci
* IN-PROGRESS notmuch
- [X] assert notmuch (seems pretty cool and good regarding gpg support)
- [X] branch in mail-pack for notmuch
- [X] Convert mail-pack to use notmuch
- [ ] Learn how to deal with tag? ~> this seems possible only by using `notmuch tag` -> find a configuration file setup
* PENDING Change laptop's broken battery jack
- [X] Disassemble laptop
- [ ] Fix broken piece
- [ ] Reassemble
...
* CANCELLED Try mbsync instead of offlineimap
CLOSED: [2015-07-23 Thu 09:14]

Offlineimap works nice.
I've cleaned up my inbox instead.
The local client (was mu, not is notmuch) do thy bidding.
...
* CANCELLED shake
CLOSED: [2015-08-01 Sat 16:16]
http://shakebuild.com/
https://github.com/ndmitchell/shake
* CANCELLED https://github.com/oblique/create_ap
CLOSED: [2015-07-23 Thu 09:15]
- [ ] Create nix expression for this

* CANCELLED thunderbird for hostile env.
CLOSED: [2015-07-23 Thu 09:16]
- [ ] exquilla
- [ ] exchange ws provider -> ericson
- [ ] lightning
* DONE gpg [4/4]
CLOSED: [2015-07-21 Tue 13:27]
- [X] Take control of your gpg key (sign, encrypt)
- [X] Import some known ones
- [X] Publish gpg public key
- [X] At least one signature, yes -> zack did it
...
- [X] Update org-trello derivation + deps
- [X] new https://github.com/aktau/github-release/releases NixOS/nixpkgs/pull/9702
...
** DONE Factor NixOS configuration [50%]
...
- [-] Make alderaan's setup shareable with other machines' [2/3]
  - [X] extract xserver's setup inside each machine for one (chris does not use xmonad nor does she know qwerty)
  - [X] Install a bunch of default packages but let the machines define their specific one (chris' machine is not a dev one)
  - [ ] Find a way to define a keyboard layout/language per user
** PENDING NixOps [0%]
...
* TODO Review [[https://github.com/jwiegley/alert][jwiegley/alert]]
[2015-08-12 Wed 11:35]
* TODO Review [[http://emacs.stackexchange.com/questions/798/recording-a-gif-screencast-of-emacs][images - Recording a GIF screencast of Emacs - Emacs Stack Exchange]]
[2015-08-12 Wed 15:03]
* TODO Review [[https://github.com/Malabarba/camcorder.el][Malabarba/camcorder.el]]
[2015-08-12 Wed 15:14]
* TODO Review [[http://www-archive.mozilla.org/unix/customizing.html#key_example][Customizing Mozilla]]
[2015-08-15 Sat 10:27]
* [[https://github.com/SynoCommunity/spksrc/wiki/Architecture-per-Synology-model][syno matrix architecture]]
* emails
** client [6/7]
- [X] Abstract further from gmail
- [X] Take back control (offlineimap, notmuch, afew)
- [X] Improve filtering (afew)
- [X] Separate emails setup (ardumont/dot-mails) from ardumont/dot-files
- [X] Deal with 'spam' and 'deleted' emails
- [X] Deploy
- [-] Improve notmuch-emacs performance [2/5]
  - [X] gmail is slow when inbox contains too much emails
  - [X] notmuch-emacs is rendering slowly because:
    - [X] too much emails in mailbox
    - [X] no pagination (nor cache)
    - [ ] Split into subfolders?
  - [ ] sieve filtering?

** Investigate running server
- dovecot, sieve
