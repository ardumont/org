...
- [X] Propose a fix about pidgin-with-plugins which miss the folder containing the plugins
- [X] Propose a fix about proxy to expose the env no_proxy variable too (not specified right now and create problems when developping local web app)
