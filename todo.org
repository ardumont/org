...
- [ ] C-; C-g - Try to send the abort signal as in `quit current action`
- [ ] Use notify-send instead of zenity? -> tested and not satisfied by the rendering (notifications piles up so slow)
