...
A small library to easily parse .authinfo files
- [X] 0.0.1
- [X] 0.0.2
- [X] 0.0.3
- [X] 0.0.4
...
- [ ] Adapt prefix key for xmonad/stumpwm depending on their run as main window manager or embedded in xephyr
...
- [X] C-c M-z from any buffer, spawns an ansi-term or go to one. C-c M-z from any ansi-term gets back to the buffer from whence you come from
