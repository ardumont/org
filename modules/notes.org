...
- Deploy first the extension
- Deploy at each step with results (so that the user can compare its output)
- FAQ with general annoyances and how to avoid them
- Tools to bootstrap a template (interactively ask question, etc...)
- tutorial videos
